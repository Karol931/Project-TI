...
app.get('/', async (req, res) => {
    try {
        let checkUser = await User.find({ fname: req.query.fname, lname: req.query.lname }).count()
        if (checkUser != 0) {
            fname = req.query.fname
            lname = req.query.lname
            res.render("index", { text: 'Witaj ' + req.query.fname + ' ' + req.query.lname, buttonText: "Wyloguj się", isLoggedIn: true })
        }
        else {
            res.render("index", { text: '', buttonText: "Zaloguj się", isLoggedIn: false })
        }
    } catch (err) {
        console.log(err)
    }
...
app.post('/', async (req, res) => {
    try {
        for (let i = 0; i < req.body.points.length; i++) {
            ...
            await game.save()
            let id = game._id
            await User.findOneAndUpdate({ fname: fname, lname: lname }, { $push: { gameID: id } })
        }
...
app.post('/login', async (req, res) => {
    if (req.body.fname != '' && req.body.lname != '') {
        try {
            let checkUser = await User.find({ fname: req.body.fname, lname: req.body.lname }).count()
            if (checkUser != 0) {
...
app.get('/register', (req, res) => {
    if (req.query.valid === 'BrakDanych') {
        ...
        res.render("register", { text: 'Użytkownik o podanych danych już istnieje' })
    }
    else {
...
app.get('/results', async (req, res) => {
    try {
        ...
        if (checkUser != 0) {
            let gameIDs = await User.findOne({ fname: fname, lname: lname }, { gameID: 1, _id: 0 })
